...
function flattenResponse(resp: any): string {
  try {
    ...
    return resp.output
      .map((item: any) => item?.content?.map((c: any) => c?.text?.value || "").join(""))
      .join("")
      .trim();
  } catch {
    return "";
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!process.env.OPENAI_API_KEY) return err("OPENAI_API_KEY is not set on the server");

    if (!finalize) {
      // Interview turn
      const chat = await client.chat.completions.create({
        model: "gpt-4o-mini", // use a model you have
        temperature: 0.2,
        messages: [
      ...
      const text = chat.choices?.[0]?.message?.content?.toString().trim() || "I didn't get a response. Please try again.";
      return ok({ text });
    }
